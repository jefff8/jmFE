...
document.addEventListener("DOMContentLoaded",function(){
	// 获取DOM元素对象
	//验收记录表
	history_zgtz=document.getElementById("history_zgtz");
	empty_zgtz=document.getElementById("empty_zgtz");		
...
function createItem( p,entry,lx ) {
	...
	if (lx=='zgtz') {
		hs=history_zgtz;
		ep=empty_zgtz;
		files_zgtz.push({name:"upfile"+index_zgtz,path:p});
		index_zgtz++;
	}
...
function cleanHistory(lx) {	
	...
	mui.confirm('您确定要清空记录吗？', '警告:', btnArray, function(e) {
		if (e.index == 0) {
			if (lx=='zgtz') {				
				history_zgtz.innerHTML = '<li id="empty_zgtz" class="ditem-empty">无历史记录</li>';
				empty_zgtz=document.getElementById("empty_zgtz");	
				files_zgtz=[];
				index_zgtz=1;
			}
...
////////////上传文件/////////////////////////////////////////////
var server=url+"my_household/household_upload2.php";
var files1=[];
var files_zgtz=[];
//上传函数
...
//			outLine("上传成功："+t.responseText);
			...
			var button_clean1=document.getElementById("clean_zgtz");
	...
	task.addData("rectification_notice",text1());
...
//验收记录表说明
function text1(){
	var rectification_notice = document.getElementById("rectification_notice").value;
	return rectification_notice;
}
