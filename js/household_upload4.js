...
document.addEventListener("DOMContentLoaded",function(){
	// 获取DOM元素对象
	//验收记录表
	history_hzb=document.getElementById("history_hzb");
	empty_hzb=document.getElementById("empty_hzb");		
...
function createItem( p,entry,lx ) {
	...
	if (lx=='hzb') {
		hs=history_hzb;
		ep=empty_hzb;
		files_hzb.push({name:"upfile"+index_hzb,path:p});
		index_hzb++;
	}
...
function cleanHistory(lx) {	
	...
	mui.confirm('您确定要清空记录吗？', '警告:', btnArray, function(e) {
		if (e.index == 0) {
			if (lx=='hzb') {				
				history_hzb.innerHTML = '<li id="empty_hzb" class="ditem-empty">无历史记录</li>';
				empty_hzb=document.getElementById("empty_hzb");	
				files_hzb=[];
				index_hzb=1;
			}
...
////////////上传文件/////////////////////////////////////////////
var server=url+"my_household/household_upload4.php";
var files1=[];
var files_hzb=[];
//上传函数
...
//			outLine("上传成功："+t.responseText);
			...
			var button_clean1=document.getElementById("clean_hzb");
	...
	task.addData("summaryText",summaryText());
