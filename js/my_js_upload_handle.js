	...
	mui.plusReady(function(){
		...
		var before_ph5=document.getElementById('before_ph5');
		before_ph5.addEventListener('tap',function () {
			myactionSheet('rwfj6');
		});	
...
//		outLine( "成功："+p );
		...
		plus.io.resolveLocalFileSystemURL( p, function ( entry ) {
			var pic=document.getElementById("my_img_id");
		  	var before_ph=document.getElementById("before_ph5");
...
function galleryImg(lx) {
	...
	plus.gallery.pick(function(p){
	  ...
	  if(lx=='rwfj6'){
	  	...
	  	var before_ph5=document.getElementById("before_ph5");
		pic.classList.remove('my_none');
		before_ph5.classList.add('my_none');
		pic.src=p;
		pic.realUrl=pic.src;
		createItem(p,'rwfj6');
	  }
...
function createItem( p,lx ) {
	if (lx=='rwfj6') {
		files_rwfj6.push({name:"upfile"+index_rwfj6,path:p});
		index_rwfj6++;
	}
}
////////////上传文件/////////////////////////////////////////////
...
//			button_clean.disabled=true;
	...
	nub6=files6.length.toString();
	task.addData("nub6",nub6);
