...
document.addEventListener("DOMContentLoaded",function(){
	// 获取DOM元素对象
    //自测照片
	hs_zc=document.getElementById("history_zczp");
	ep_zc=document.getElementById("empty_zczp");
...
//var server="http://demo.dcloud.net.cn/helloh5/uploader/upload2.php";
var files=[];
var files_zczp=[];
////////////上传文件/////////////////////////////////////////////
...
function createItem( p,entry,lx ) {
	...
	if (lx=='zczp') {
		hs=hs_zc;
		ep=ep_zc;
		files_zczp.push({name:"upfile"+index_zczp,path:p});
		index_zczp++;
	}
...
function cleanHistory(lx) {	
	...
	mui.confirm('您确定要清空记录吗？', '警告:', btnArray, function(e) {
		if (e.index == 0) {
			if (lx=='zczp') {				
				hs_zc.innerHTML = '<li id="empty_zczp" class="ditem-empty">无历史记录</li>';
				ep_zc=document.getElementById("empty_zczp");	
				files_zczp=[];
				index_zczp=1;
			}
