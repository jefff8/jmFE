	...
	mui.plusReady(function(){
		...
		var newUpload5=document.getElementById('newUpload5');
		newUpload5.addEventListener('tap',function () {
			getImage('clzp');
		});	
	});
...
document.addEventListener("DOMContentLoaded",function(){
	// 获取DOM元素对象
	//处理照片
	hs_clzp=document.getElementById("history_clzp");
	ep_clzp=document.getElementById("empty_clzp");		
...
function createItem( p,entry,lx ) {
	...
	if (lx=='clzp') {
		hs=hs_clzp;
		ep=ep_clzp;
		files_clzp.push({name:"upfile"+index_clzp,path:p});
		index_clzp++;
	}
...
function cleanHistory(lx) {	
	...
	mui.confirm('您确定要清空记录吗？', '警告:', btnArray, function(e) {
		if (e.index == 0) {
			if (lx=='clzp') {				
				hs_clzp.innerHTML = '<li id="empty_clzp" class="ditem-empty">无历史记录</li>';
				ep_clzp=document.getElementById("empty_clzp");	
				files_clzp=[];
				index_clzp=1;
			}
...
////////////上传文件/////////////////////////////////////////////
//上传
var server=url+"my_plus/pic_upload_SelfTest.php";
//var server="http://demo.dcloud.net.cn/helloh5/uploader/upload.php";
var files=[];
var files_clzp=[];
function upload4(lx,clean){	
